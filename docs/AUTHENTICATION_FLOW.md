...
sequenceDiagram
    ...
    Browser->>Auth: GET /oauth2/authorize?<br/>response_type=code<br/>&client_id=abstratium-abstoggle<br/>&redirect_uri=http://localhost:8087/oauth/callback<br/>&state=[random]<br/>&code_challenge=[hash]<br/>&code_challenge_method=S256<br/>&scope=openid+profile+email
    ...
    OIDC->>Auth: POST /oauth2/token<br/>grant_type=authorization_code<br/>&code=[auth_code]<br/>&redirect_uri=http://localhost:8087/oauth/callback<br/>&code_verifier=[verifier]<br/>Authorization: Basic [client_id:secret]
...
https://auth.abstratium.dev/oauth2/authorize?
  response_type=code
  &client_id=abstratium-abstoggle
  &redirect_uri=http://localhost:8087/oauth/callback
...
public class DemoResource {
    ...
    @GET
    @RolesAllowed({Roles.USER})  // Requires "abstratium-abstoggle_user" role
...
**Role Mapping:**
- JWT contains: `"groups": ["abstratium-abstoggle_user"]`
...
quarkus.http.cors.origins=http://localhost:4207,http://localhost:8087
...
# Client Credentials
quarkus.oidc.client-id=abstratium-abstoggle
...
**Cause:** Role mismatch - JWT has `abstratium-abstoggle_user` but code checks for a different role.
